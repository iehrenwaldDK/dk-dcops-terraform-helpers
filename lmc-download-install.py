...
LM_API_CFG = logicmonitor_sdk.Configuration()

try:
    options, arguments = getopt.getopt(sys.argv[1:], "",
                                       ['access-id=', 'access-key=', 'company=',
                                        'collector-id=', 'collector-ea=',
                                        'collector-esc-chain=', 'collector-failover=',
                                        'collector-group-id=', 'collector-os-and-arch=',
                                        'collector-size=', 'collector-version=', 'collector-group-ab=',
                                        'snmp-security=', 'snmp-auth=', 'snmp-auth-token=',
                                        'snmp-priv=', 'snmp-priv-token=',
...
for opt, arg in options:
    print(f"{opt}={arg}\n")
    if opt in '--company':
        LM_API_CFG.company = arg
        COMPANY_SET = True
    elif opt in '--access-id':
        LM_API_CFG.access_id = arg
    elif opt in '--access-key':
        LM_API_CFG.access_key = arg
    ...
    elif opt in '--collector-esc-chain':
        COLLECTOR_ESC_CHAIN = arg
    ...
    elif opt in '--collector-failover':
        COLLECTOR_FAILOVER = arg

if not LM_API_CFG.access_id or \
   not LM_API_CFG.access_key or \
   not COMPANY_SET or \
   not COLLECTOR_ID:
    print("Not all required info is present, exiting")
    sys.exit(1)

lm_api = logicmonitor_sdk.LMApi(logicmonitor_sdk.ApiClient(LM_API_CFG))
...
#  to use it
if COLLECTOR_ESC_CHAIN:
    print("Finding requested collector-down escalation chain")
    escalation_chain_filter = 'name:"' + COLLECTOR_ESC_CHAIN + '"'

    try:
        GECL_response = lm_api.get_escalation_chain_list(
    ...
    try:
        GCBID_response = lm_api.get_collector_by_id(id=COLLECTOR_ID)
    except ApiException as e:
...
#  enable it
if COLLECTOR_GROUP_AB == "true":
    print(f"Finding collector group id {GCBID_response.collector_group_id} for autobalancing")
    try:
        GCGBID_response = lm_api.get_collector_group_by_id(GCBID_response.collector_group_id)
    except ApiException as e:
...
try:
    GDBID_response = lm_api.get_device_by_id(GCBID_response.collector_device_id)
except ApiException as e:
